...
const TEST_PREFIX = `selenium_multi_proper_${Date.now()}`;
...
// Device configurations - proper viewport sizes
const devices = [
  { name: 'Mobile', width: 375, height: 667, viewport: { width: 375, height: 667 } },
  { name: 'Tablet', width: 768, height: 1024, viewport: { width: 768, height: 1024 } },
  { name: 'Desktop', width: 1920, height: 1080, viewport: { width: 1920, height: 1080 } }
];
...
async function runTestsForDevice(device) {
  ...
  const chromeOptions = new chrome.Options()
    ...
    .addArguments(`--window-size=${device.width},${device.height}`)
  ...
  try {
    ...
    // 1. Open homepage
    try {
      await driver.get('http://localhost:5177');
    ...
    try {
      ...
      addTest(device.name, '2. 窗口尺寸', true, `${windowSize.width}x${windowSize.height}`);
    } catch (e) {
      addTest(device.name, '2. 窗口尺寸', false, e.message);
    }
    await takeScreenshot(driver, device.name, '02_viewport');
    ...
    try {
      ...
      addTest(device.name, '5. 帖子列表', true, `Found ${cards.length} cards`);
    } catch (e) {
      addTest(device.name, '5. 帖子列表', false, e.message);
    }
    ...
    try {
      ...
      if (nextBtns.length > 0) {
        ...
        if (!isDisabled) {
          await nextBtn.click();
          await sleep(1000);
          addTest(device.name, '6. 翻页功能', true, 'Navigated to page 2');
        } else {
          addTest(device.name, '6. 翻页功能', false, 'Next button disabled');
        }
      } else {
        addTest(device.name, '6. 翻页功能', false, 'No pagination found');
      }
    } catch (e) {
      addTest(device.name, '6. 翻页功能', false, e.message);
    }
...
async function runAllTests() {
  console.log('🔄 Starting Multi-Device Selenium E2E Tests (with 100 posts)...\n');
  ...
  console.log(`Prefix for PDF: ${TEST_PREFIX}`);
