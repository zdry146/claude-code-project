    ...
    stage('Checkout GitHub->Gitee fallback') {
        ...
        try {
            timeout(time: 30, unit: 'SECONDS') {
                sh '''#!/bin/bash
                    ...
                    cd ''' + workspace + '''
                    rm -rf .cloned-tmp
                    git clone --depth=1 -b master https://github.com/zdry146/claude-code-project.git .cloned-tmp
            ...
            githubOk = true
            gitUrl = 'https://github.com/zdry146/claude-code-project.git'
        } catch (Exception e) {
        ...
        if (!githubOk) {
            ...
            sh '''#!/bin/bash
                ...
                cd ''' + workspace + '''
                rm -rf .cloned-tmp
                git clone --depth=1 -b master git@gitee.com:zdry146/claude-code-project.git .cloned-tmp
                shopt -s dotglob
                cp -a .cloned-tmp/. .
                rm -rf .cloned-tmp
            '''
            gitUrl = 'git@gitee.com:zdry146/claude-code-project.git'
        }
